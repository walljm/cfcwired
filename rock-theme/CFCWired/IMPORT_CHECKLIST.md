...
- [ ] If the active Rock theme remains `CFCv2External`, run `npm run build:rock-css` and merge or append `Styles/cfc-site-overrides.css` into `/Themes/CFCv2External/Styles/theme.css`, or load it after `theme.css`.
- [ ] Confirm Rock still loads its own Bootstrap 3 CSS and jQuery bundles.
- [ ] Confirm the theme CSS loads after Rock Bootstrap CSS.

## 2. Assets

- [ ] Upload or copy page images from `src/images/` to `/Content/ExternalSite/Images/`.
- [ ] Confirm the header logo resolves at `/Content/ExternalSite/Media/cfc-logo-light.png`.
- [ ] Confirm footer app badge images resolve at their configured `/Content/ExternalSite/Media/` paths.
...
- [ ] Paste each content-page `feature.html` into the listed `Feature` zone.
- [ ] Paste each content-page `main.html` into the listed `MainContent` zone.
...
- [ ] Homepage events: configure Calendar Item Occurrence List by Audience Lava. See `NativeBlocks/homepage-events.md`.
